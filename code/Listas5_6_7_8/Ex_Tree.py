...
def imprimeArvore(tree, nivel=0, lado='*'):
    if tree is not None :
        head = "└──"
        espacoPai = "    "
        espacoFilho = " " 
        for _ in range(nivel):
            espacoPai += " "
            espacoFilho += " "
        print(espacoPai + espacoFilho + head+ lado + "(" + str (tree.data) + str(")"))
        imprimeArvore(tree.right, nivel +1, 'r')
        imprimeArvore(tree.left, nivel +1, 'l')
...
if __name__ == '__main__':
    ...
    pre = [ 'B', 'M', 'K', 'A', 'C', 'J', 'L', 'Z', 'N', 'F', 'W', 'D', 'Y', 'E']
    inn = [ 'A', 'C', 'K', 'M', 'L', 'J', 'Z', 'B', 'F', 'N', 'D', 'Y', 'W', 'E']
    lenn = len(inn)
